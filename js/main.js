{
    ...
    class Slideshow {
		...
		update(newEntry) {
			const updateFn = () => {
				...
				// when both hide and show are finished:
				Promise.all([this.currentEntry.hide(this.direction), newEntry.show(this.direction)]).then(() => {
	...
	imagesLoaded(document.querySelectorAll('img'), () => {
		...
		document.addEventListener('click', function(event) {
			if (!event.target.matches('.ga-link')) return;
			event.preventDefault();
			ga('send', 'event', {
				eventCategory: 'Outbound Link',
				eventAction: 'click',
				eventLabel: 'Pre-order link'
			});
		}, false);
	});
};